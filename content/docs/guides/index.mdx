...
API транскрибации принимает на вход аудио (или видео) файл, который вы хотите транскрибировать (либо ссылку на этот файл), и желаемый формат вывода. Модель поддерживает следующие форматы вывода (`json`, `text`, `srt`, `verbose_json`, `vtt`). Подробнее о форматах вывода читайте на странице [Форматы ответа](#форматы-ответа).
...
Хотя модель была обучена на 98 языках, список выше содержит только языки, у которых Word Error Rate (WER) не больше 50%. Полный список языков и их кодов в формате ISO-639-1 смотрите на странице [Supported Languages](#поддерживаемые-языки).
